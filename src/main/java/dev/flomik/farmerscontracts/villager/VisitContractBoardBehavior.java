...
public class VisitContractBoardBehavior extends Behavior<Villager> {
    ...
    public VisitContractBoardBehavior(float speed) {
        super(ImmutableMap.of(ContractVillagerMemories.NEAREST_BOARD, MemoryStatus.VALUE_PRESENT), RUN_TIME_TICKS);
        this.speed = speed;
    }

    @Override
    protected boolean canStillUse(ServerLevel level, Villager villager, long gameTime) {
        return villager.getBrain().getMemory(ContractVillagerMemories.NEAREST_BOARD)
    ...
    protected void tick(ServerLevel level, Villager villager, long gameTime) {
        villager.getBrain().getMemory(ContractVillagerMemories.NEAREST_BOARD)
                .ifPresent(pos -> BehaviorUtils.setWalkAndLookTargetMemories(villager, pos, speed, CLOSE_ENOUGH_DIST));
    }

    @Override
    protected void stop(ServerLevel level, Villager villager, long gameTime) {
        villager.getBrain().eraseMemory(ContractVillagerMemories.NEAREST_BOARD);
    }
}
