...
public class ContractBoxMenu extends AbstractContainerMenu {
    public static final StreamCodec<RegistryFriendlyByteBuf, List<Item>> ALLOWED_ITEMS_STREAM_CODEC =
    ...
    public ContractBoxMenu(int containerId, Inventory playerInventory) {
        this(containerId, playerInventory, (List<Item>) null);
    }

    public ContractBoxMenu(int containerId, Inventory playerInventory, @Nullable List<Item> allowedItems) {
        this(containerId, playerInventory, new PredictionSizedContainer(ContractBoxBlockEntity.SLOTS),
                insertFilterOf(allowedItems));
    }

    private static Predicate<ItemStack> insertFilterOf(@Nullable List<Item> allowedItems) {
        if (allowedItems == null) {
            return stack -> true;
        }
        Set<Item> allowed = Set.copyOf(allowedItems);
        return stack -> ContractBoxBlockEntity.mayContain(stack, allowed);
    }
